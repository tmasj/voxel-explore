...
fn draw_frame_once(vk_ctx: &VulkanContext) {
    unsafe {
        ...
        vk_ctx.device.queue_submit(vk_ctx.queue, &submits, vk::Fence::null())
...
fn record_command_buffer(vk_ctx: &VulkanContext, image_index: u32, frame_index: usize) {
    ...
    unsafe{
        ...
        vk_ctx.device.cmd_set_viewport(cmd_buffer_target, 0, &viewports);
...
fn get_triangle_geometry() -> Vec<Vertex> {
    vec![
        Vertex { position: [0.0, -0.5, 0.0], color: [1.0, 0.0, 0.2] },
        Vertex { position: [0.5, 0.5, 0.0], color: [0.1, 0.5, 1.0] },
        Vertex { position: [-0.5, 0.5, 0.0], color: [0.7, 0.5, 0.0] },
    ]
}

fn get_triangle_geometry2() -> Vec<Vertex> {
    vec![
        Vertex { position: [0.5, -0.5, 0.0], color: [1.0, 0.0, 0.2] },
        Vertex { position: [0.0, 0.5, 0.0], color: [0.1, 0.5, 1.0] },
        Vertex { position: [0.5, 0.5, 0.0], color: [0.7, 0.5, 0.0] },
    ]
}
