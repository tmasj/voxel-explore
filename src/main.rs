...
impl ProgramHead {
    fn new_from_current_instant() -> () {
        ...
        let graphics = VulkanLifecycle::new();
        let game_global = GameGlobal::new_game_current_instant();
        event_loop(&mut glfwh, &mut window, events, &mut vk_ctx);
        // game_global.event_loop();
        return ();
    }
}
...
impl GlfwKernel {
    fn detect_platform() -> glfw::Platform {
    ...
    fn vulkan_extension_names(self: &Self) -> Vec<CString> {
        self.glfw_handle
    ...
    fn new_window(self: &mut Self) -> (PWindow, WindowEvents) {
    ...
    fn new_from_current_platform() -> Self {
...
impl GameGlobal {
    fn new_game_current_instant() -> Self {
        Self {
    ...
    // ) {}
    fn event_loop(windowing: &WindowLifecycle, hw_accel: &VulkanLifecycle) {
        let mut frameidx = 0;
...
impl VulkanKernel {
    fn new_vulkan_instance(entry: &ash::Entry, extension_names: &Vec<CString>) -> ash::Instance {
    ...
    fn new_from_glfw(glfw_kernel: &GlfwKernel) -> Self {
    ...
    fn new_surface(
        ...
        // TODO I believe the Instance's latent state in the driver is mutated by this fn, so that should be reflected in the argument if so.
    ...
    fn new_logical_device_and_queue(
        self: &Self,
...
impl VulkanDeviceContext {
    fn new(vulkan_kernel: &VulkanKernel, windowing: &WindowLifecycle) -> Self {
    ...
    fn load_index_data_via_staging_buffer(
        vk_ctx: &VulkanContext,
...
impl VulkanLifecycle {
    fn new(glfw_context: &GlfwKernel, windowing: WindowLifecycle) -> VulkanLifecycle {
        let vk_kernel = VulkanKernel::new_from_glfw(glfw_kernel);
        let device_ctxt = VulkanDeviceContext::new(&vk_kernel, windowing);

        let swapchain = Swapchain::new_basic(
            &instance,
            &device,
            physical_device,
            &surface_loader,
